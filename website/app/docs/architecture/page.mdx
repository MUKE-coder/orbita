...
Everything below runs on a single VPS. Traffic enters through Traefik, which terminates TLS and
routes to Orbita — one Go binary that runs the API, serves the dashboard, and orchestrates every
tenant app on Docker Swarm.
...
| Component | Role |
|---|---|
| **Control plane** (Go binary) | REST API `/api/v1`, embedded React 19 dashboard, cron scheduler, WebSocket hub |
...
JWT · Resend · robfig/cron · gorilla/websocket · golang-migrate · zerolog.
...
- [Quickstart](/docs/quickstart) — deploy in two commands.
- [Install on a fresh server](/docs/quickstart) — the manual, step-by-step path.
