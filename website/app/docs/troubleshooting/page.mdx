...
Then check the firewall and Traefik on the box itself:
...
- Inspect: `orbita logs --host prod --service migrate`.
...
Usually a domain typo in `orbita.yaml` (must be a **bare hostname** — no scheme/port/path) or DNS
not yet propagated.

## `git push` doesn't auto-deploy

- The webhook needs `admin:repo_hook` scope — re-run `orbita github-auth` with a token that
  has it.
